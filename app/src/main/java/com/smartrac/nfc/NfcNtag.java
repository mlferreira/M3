...
public class NfcNtag implements TagTechnology {
    ...
    public byte[] amiiboFastRead(int startAddr, int endAddr, int bank) {
        ...
        while ((i < iNumReads) && bOk)
        {
            ...
            byte[] respSnippet = amiiboFastReadHelper(startSnippet, endSnippet, bank);
            if (respSnippet == null)
            {
                bOk = false;
            }
    ...
    private byte[] amiiboFastReadHelper(int startAddr, int endAddr, int bank) {

        byte[] req = new byte[4];
        byte[] resp;

        req[0] = NfcNtagOpcode.AMIIBO_FAST_READ;
        req[1] = (byte)(startAddr & 0xFF);
        req[2] = (byte)(endAddr & 0xFF);
        req[3] = (byte)(bank & 0xFF);
